...
const EMAIL_TEMPLATES = {
  VERIFICATION: {
    subject: "Verify Your ViewerApps Account",
    textPart: (verificationLink: string) => 
      `Welcome to ViewerApps!\n\n` +
      `Please verify your email address by clicking the link below:\n` +
      `${verificationLink}\n\n` +
      `This link will expire in 24 hours.\n\n` +
      `If you did not create an account with ViewerApps, please ignore this email.\n\n` +
      `The ViewerApps Team`,
    htmlPart: (verificationLink: string) => 
      `<div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        <div style="background: linear-gradient(to right, #3b82f6, #2563eb); padding: 20px; border-radius: 5px 5px 0 0;">
          <h1 style="color: white; margin: 0; font-size: 24px; text-align: center;">Welcome to ViewerApps</h1>
        </div>
        <div style="padding: 20px; border: 1px solid #e5e7eb; border-top: none; border-radius: 0 0 5px 5px;">
          ...
          <p style="font-size: 14px; line-height: 24px; color: #6b7280;">If you did not create an account with ViewerApps, please ignore this email.</p>
          <hr style="border: none; border-top: 1px solid #e5e7eb; margin: 30px 0;" />
          <p style="font-size: 12px; color: #9ca3af; text-align: center;">© ${new Date().getFullYear()} ViewerApps. All rights reserved.</p>
        </div>
      </div>`
  },
  WELCOME: {
    subject: "Welcome to ViewerApps!",
    textPart: (username: string) => 
      `Hi ${username},\n\n` +
      `Thank you for joining ViewerApps! We're excited to have you onboard.\n\n` +
      `You can now access your dashboard to start growing your streaming audience.\n\n` +
    ...
    htmlPart: (username: string) => 
      `<div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <div style="padding: 20px; border: 1px solid #e5e7eb; border-top: none; border-radius: 0 0 5px 5px;">
          ...
          <p style="font-size: 16px; line-height: 24px; color: #374151;">Thank you for joining ViewerApps! We're excited to have you onboard.</p>
          <p style="font-size: 16px; line-height: 24px; color: #374151;">You can now access your dashboard to start growing your streaming audience.</p>
          <div style="text-align: center; margin: 30px 0;">
            <a href="https://viewerapps.com/app" style="background: linear-gradient(to right, #3b82f6, #2563eb); color: white; padding: 12px 30px; text-decoration: none; border-radius: 5px; font-weight: bold; display: inline-block;">Go to Dashboard</a>
          </div>
          <p style="font-size: 16px; line-height: 24px; color: #374151;">If you have any questions, feel free to contact our support team.</p>
          <p style="font-size: 16px; line-height: 24px; color: #374151; margin-top: 30px;">Best regards,<br>The ViewerApps Team</p>
          <hr style="border: none; border-top: 1px solid #e5e7eb; margin: 30px 0;" />
          <p style="font-size: 12px; color: #9ca3af; text-align: center;">© ${new Date().getFullYear()} ViewerApps. All rights reserved.</p>
        </div>
      </div>`
