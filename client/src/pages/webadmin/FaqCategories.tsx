...
const FaqCategories: React.FC = () => {
  ...
  // Fetch categories
  const { data: categories, isLoading, isError, error } = useQuery<FaqCategory[]>({
  ...
  const createMutation = useMutation({
    mutationFn: async (data: typeof formData) => {
      const res = await apiRequest('POST', '/api/admin/faq-categories', data);
      return res.json();
    },
    onSuccess: () => {
      toast({
    ...
    onError: (error: any) => {
      toast({
        title: 'Error',
        description: error.message || 'Failed to create FAQ category',
        variant: 'destructive',
      });
    }
  });
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, data }: { id: number; data: typeof formData }) => {
      const res = await apiRequest('PUT', `/api/admin/faq-categories/${id}`, data);
      return res.json();
    },
    onSuccess: () => {
      toast({
    ...
    onError: (error: any) => {
      toast({
        title: 'Error',
        description: error.message || 'Failed to update FAQ category',
        variant: 'destructive',
      });
    }
  });
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: number) => {
      const res = await apiRequest('DELETE', `/api/admin/faq-categories/${id}`);
      return res.json();
    },
    onSuccess: () => {
      toast({
  ...
  const openDeleteModal = (category: FaqCategory) => {
    setCurrentCategory(category);
      ...
      <AdminLayout>
        <div className="p-4">
          <div className="bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded">
            <p>Error loading FAQ categories: {(error as Error)?.message || 'Unknown error'}</p>
          </div>
        </div>
      </AdminLayout>
    );
      ...
      <div className="p-6 max-w-7xl mx-auto">
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <p className="text-muted-foreground">Manage FAQ categories for organizing frequently asked questions</p>
          </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle>All FAQ Categories</CardTitle>
            <CardDescription>
              Categories are used to organize FAQ items. Categories are displayed in order of their position.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <Table>
                ...
                {categories && categories.length > 0 ? (
                  ...
                  <TableRow>
                    <TableCell colSpan={6} className="text-center py-8">
                      No FAQ categories found. Click the "Add Category" button to create one.
                    </TableCell>
                  </TableRow>
                )}
              </TableBody>
        ...
        <Dialog open={isEditModalOpen} onOpenChange={setIsEditModalOpen}>
          <DialogContent className="sm:max-w-[500px]">
            <DialogHeader>
              <DialogTitle>{currentCategory ? 'Edit FAQ Category' : 'Create New FAQ Category'}</DialogTitle>
              <DialogDescription>
                {currentCategory ? 'Update the category details' : 'Fill in the details to create a new FAQ category'}
              </DialogDescription>
            </DialogHeader>
              ...
              <div className="grid gap-4 py-4">
                ...
                <div className="grid gap-2">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Categories with lower position values appear first.
                  </p>
                </div>
        ...
        <Dialog open={isDeleteModalOpen} onOpenChange={setIsDeleteModalOpen}>
          <DialogContent className="sm:max-w-[425px]">
            ...
            <div className="py-4">
              <p>
                Are you sure you want to delete the FAQ category "{currentCategory?.name}"?
                Any FAQs in this category will be unassigned.
              </p>
            </div>
            <DialogFooter>
              <Button
